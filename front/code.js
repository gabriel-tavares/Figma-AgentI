...
function hexFromPaint(paint) {
    ...
    if (!c)
        return null;
    const to255 = (v) => Math.round(Math.min(Math.max(v * 255, 0), 255));
            ...
            try {
                ...
                if (isMixedSize || isMixedFont) {
                    ...
                    while (i < len) {
                        ...
                        const fnr = t.getRangeFontName(i, i + 1);
                        let j = i + 1;
                        while (j < len) {
                            const fs2 = t.getRangeFontSize(j, j + 1);
                            const fn2 = t.getRangeFontName(j, j + 1);
...
function getAbsXY(n) {
    const m = n.absoluteTransform;
    const tx = Array.isArray(m) && m.length > 0 ? (m[0][2] || 0) : (n.x || 0);
    const ty = Array.isArray(m) && m.length > 1 ? (m[1][2] || 0) : (n.y || 0);
    return { x: tx, y: ty };
}
function boundsRelativeTo(frame, n) {
    ...
    const pct = (v, total) => total > 0 ? Math.max(0, Math.min(1, v / total)) : 0;
    return {
        ...
        if ('fills' in frame) {
            ...
            if (Array.isArray(fills)) {
                for (const p of fills) {
                    const h = hexFromPaint(p);
                    if (h && bg.indexOf(h) < 0)
                        bg.push(h);
                }
            }
        }
        const nodes = frame.findAll ? frame.findAll(() => true) : [];
...
function textStyleOf(t) {
    try {
        ...
        const sizePx = typeof t.fontSize === 'number' ? Math.round(t.fontSize) : null;
        const lhObj = t.lineHeight;
        const lh = (lhObj && typeof lhObj === 'object' && lhObj.unit === 'PIXELS') ? Math.round(Number(lhObj.value || 0)) : null;
        const lsObj = t.letterSpacing;
        const ls = (lsObj && typeof lsObj === 'object') ? Number(lsObj.value || 0) : null;
        const align = String(t.textAlignHorizontal || '').toLowerCase();
        const decoration = String(t.textDecoration || 'NONE').toLowerCase();
...
async function buildFigmaSpecFromFrame(frame, nomeLayoutOverride) {
    const W = frame.width || 0;
    const H = frame.height || 0;
    const accFamilies = new Set();
    const scale = [];
    ...
    try {
        ...
        const sorted = texts.map(t => { const fs = (typeof t.fontSize === 'number') ? t.fontSize : 0; return { t, sz: fs }; }).sort((a, b) => b.sz - a.sz).slice(0, 5);
        headers = sorted.map(x => String((x.t.characters || '')).slice(0, 120)).filter(Boolean);
    }
    catch (e) { }
    ...
    try {
        ...
        for (const n of all) {
            const name = String(n.name || '').toLowerCase();
            if (n.type === 'VECTOR' || name.indexOf('icon') >= 0) {
                const key = name || 'icon';
    ...
    try {
        ...
        for (const n of all) {
            ...
            const label = (n.type === 'TEXT') ? (n.characters || 'sem rotulo') : (String(n.name || 'sem rotulo'));
            const geo = boundsRelativeTo(frame, n);
            // --- Auto Layout do pai: padding e itemSpacing (se houver)
            let __parentPad = { paddingTopPx: null, paddingRightPx: null, paddingBottomPx: null, paddingLeftPx: null,
    ...
    return {
        ...
        contrastPairs: [],
        typography: { families: Array.from(accFamilies), scaleRaw: scale, scale: scaleAgg, totals: { textNodes: frame.findAll((n) => n.type === 'TEXT').length, uniqueCombos: scaleAgg.length } },
        iconography,
        mainText: headers,
        components: comps
    };
...
function contarLayoutsSelecionados() {
    ...
    const selecionados = figma.currentPage.selection.filter(n => n.type === "FRAME" ||
        n.type === "INSTANCE" ||
        n.type === "COMPONENT" ||
...
async function createBenchmarkCards(results, node) {
    try {
        // Carregar fontes
        await Promise.all([
...
async function makeSeverityChip(meta) {
...
function tweenOpacitySafe(node, from, to, duration = 180, onDone) {
...
figma.ui.onmessage = async (msg) => {
    ...
    if (msg.type === "getCardsVisibilityState") {
        try {
            const cards = figma.currentPage.findAll(n => typeof n.name === "string" && n.name.trim().startsWith("[AI]"));
            if (cards.length === 0) {
    ...
    if (msg && msg.type === "setHeuristicaVisibility") {
        ...
        const nodes = figma.currentPage.findAll(n => typeof n.name === "string" &&
            n.name.indexOf("[AI]") === 0);
        let count = 0;
        for (let i = 0; i < nodes.length; i++) {
            ...
            try {
                if (visible) {
                    ...
                    const from = hasOpacity(n) ? n.opacity : 1;
    ...
    if (msg && msg.type === "deleteAllHeuristicaCards") {
        const nodes = figma.currentPage.findAll(n => typeof n.name === "string" && n.name.trim().startsWith("[AI]"));
        var removed = 0;
    ...
    if (msg && msg.type === "benchmark-multi-ai") {
        const categoria = msg.categoria || "free";
        const testType = msg.testType || "layout-analysis";
        ...
        try {
            ...
            // PRODUÇÃO: usar servidor Coolify com domínio personalizado
            const response = await fetch("https://agenti.uxday.com.br/benchmark-multi-ai", {
                method: "POST",
    ...
    if (!msg || msg.type !== "analisar")
        return;
    const metodo = msg.metodo || "";
    const descricao = msg.descricao || "";
    const selection = figma.currentPage.selection;
    if (!selection.length) {
        figma.ui.postMessage({
            carregando: false,
    ...
    function frameLooksLikeScreenshot(frame) {
        try {
            const hasText = !!frame.findOne((n) => n.type === 'TEXT');
            const hasImg = !!frame.findOne((n) => ('fills' in n) && Array.isArray(n.fills) && n.fills.some((f) => f && f.type === 'IMAGE'));
    ...
    let layoutNameFromFigma;
    if (orderedSelection.length === 1) {
        const n = orderedSelection[0];
    ...
    for (let i = 0; i < orderedSelection.length; i++) {
        ...
        if (isFrameLike(node)) {
            ...
            const spec = await buildFigmaSpecFromFrame(node, orderedSelection.length === 1 ? layoutNameFromFigma : undefined);
...
8 - ${Array.isArray(achado.referencias) ? achado.referencias.join(', ') : achado.referencias || ''}`;
        ...
        // Dividir blocos entre os layouts selecionados
        const blocosPerLayout = Math.ceil(blocos.length / orderedSelection.length);
        // Processar cada layout com sua porção de blocos
        for (let layoutIdx = 0; layoutIdx < orderedSelection.length; layoutIdx++) {
            const node = orderedSelection[layoutIdx];
            const OFFSET_X = 80;
            let currentY = node.y;
            // Pegar apenas a porção de blocos para este layout
            const startIdx = layoutIdx * blocosPerLayout;
            const endIdx = Math.min(startIdx + blocosPerLayout, blocos.length);
            const blocosDesteLayout = blocos.slice(startIdx, endIdx);
            // Criar Auto Layout para este layout
            const autoLayout = figma.createFrame();
            autoLayout.name = `[AI] ${node.name || 'Layout'}`;
            ...
            autoLayout.strokes = [];
            autoLayout.x = node.x + node.width + OFFSET_X;
            autoLayout.y = node.y;
            figma.currentPage.appendChild(autoLayout);
            let cardsPayload = [];
            // Processar blocos deste layout como partes individuais
            const todasPartes = [];
            for (let i = 0; i < blocosDesteLayout.length; i++) {
                const blocoOriginal = blocosDesteLayout[i] || "";
